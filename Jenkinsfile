      ...
      steps {
        parallel(
          ...
          "Resource regression test": {
            node(label: 'docker') {
              script {
                sh '''docker run -i --rm --name="$BUILD_TAG-reg" -e GIT_SRC="https://github.com/eea/$GIT_NAME.git" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e JS_ORG="20" -e CSS_ORG="20" -e REG_URL="https://land.copernicus.eu/" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/resourcetest:0.1'''
              }
            }
          }
        )
